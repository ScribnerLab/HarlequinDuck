...
MODEL = "MERefuge";
parmnames = c("WNE", "ENE", "BOTTN", "ANCN", "MIG", paste0(rep("TDIV",7),c(1:7)), "MIGSCAL", "ANCRAT", "GROWRATEE", "GROWRATEW");
nsims = 1000000;
...
TDIV7 = round(runif(nsims, 20000, 30000));

BOTTN = c();
ANCN = c();
TDIV1to3 = matrix(data = NA, nrow = nsims, ncol = 3);
TDIV4to5 = matrix(data = NA, nrow = nsims, ncol = 2);

for(x in 1:nsims) {
  BOTTN[x] = round(runif(1, 20, min(WNE[x], ENE[x])));
  ANCN[x] = round(runif(1, max(WNE[x], ENE[x]), 100000));
  TDIV4to5[x,] = round(sort(runif(2, 1600, TDIV6[x])));
  TDIV1to3[x,] = round(sort(runif(3, 400, min(TDIV4to5[x,]))));
}
...
params = data.frame(WNE, ENE, BOTTN, ANCN, MIG, TDIV1to3, TDIV4to5, TDIV6, TDIV7, MIGSCAL, ANCRAT, GROWRATEE, GROWRATEW);
names(params) = parmnames;

write.table(params, paste0("SSR_", MODEL, ".def"), quote = FALSE, row.names = FALSE, col.names = TRUE);

DNAparams = data.frame(round(WNE*0.25), round(ENE*0.25), round(BOTTN*0.25), round(ANCN*0.25), MIG*MIGSCAL, TDIV1to3, TDIV4to5, TDIV6, TDIV7, MIGSCAL, ANCRAT, GROWRATEE, GROWRATEW);
