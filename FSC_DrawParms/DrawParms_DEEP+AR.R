...
MODEL = "DEEP+AR";
parmnames = c("WNE", "ENE", "ARNE", "BOTTN", "ANCN", "MIG", paste0(rep("TDIV",7),c(1:7)), "MIGSCAL", "ANCRAT", "GROWRATEE", "GROWRATEW");
nsims = 1000000;
...
for(x in 1:nsims) {
  ...
  TDIV6[x] = round(runif(1, 20000, TDIV7[x]));
...
params = data.frame(WNE, ENE, ARNE, BOTTN, ANCN, MIG, TDIV1to4, TDIV5, TDIV6, TDIV7, MIGSCAL, ANCRAT, GROWRATEE, GROWRATEW);
names(params) = parmnames;

write.table(params, paste0("SSR_", MODEL, ".def"), quote = FALSE, row.names = FALSE, col.names = TRUE);

DNAparams = data.frame(round(WNE*0.25), round(ENE*0.25), round(ARNE*0.25), round(BOTTN*0.25), round(ANCN*0.25), MIG*MIGSCAL, TDIV1to4, TDIV5, TDIV6, TDIV7, MIGSCAL, ANCRAT, GROWRATEE, GROWRATEW);
